...
class ThemesPagerFragment : BasePagerFragment() {
    private val args: ThemesPagerFragmentArgs by navArgs()

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        activity?.toolbar?.subtitle = args.theme
        return super.onCreateView(inflater, container, savedInstanceState)
    }

    override fun filterQuestions(list: List<Question>): List<Question> =
        list.filter { it.theme == args.theme }
}